...
Signal::Signal (int bits, rval value)
{
        ...
        l = 0;
        makemask();
        if ((value & (~mask)) != 0) cout << "// WARNING: value wider than its bit width: " << value << endl;
        sval << dec << "(" << value << " & 0x" << hex << mask << ")";
        Signal::init(bits - 1, 0, (fullname = sval.str()).c_str());
#else   
...
//		sscanf (val.c_str(), "%d'%c%x", &bits, &radix, &value);
	...
	case 'd':
	case 'D': 
		sscanf (val.c_str(), "%d'%c%d", &bits, &radix, &ivalue);
		value = ivalue;
		break;
	case 'o': 
	case 'O': 
...
void Signal::makemask(int hpar, int lpar) 
{
	int i;
	int lng = hpar - lpar + 1;
	if ((lng >= 0) && ((unsigned)lng < Sizeofrval * 8)) 
	{
